...
GET_FORCE_DELAY = 0.1 #delay between each force sensor read
PRINT_FORCE = True
pad_channel = 0
measured_weight = 10
measured_bit = 1023
...
def ConCheck():
    ...
    if Confidence is None:
        ...
        Object = "Object: " + Guess + " Confidence: " + str(Confidence) + " Distance: " + str(Distance) + " cm"
    else:
...
def PressUpdate():
    if (PressureB is None or PressureN is None):
        ...
        Pressure_Value.config(text = str(PressureB) + " bits " + str(PressureN) + " Newtons")
...
def main():
    ...
    Overview[6][0]="Cycle Complete"
    '''
    #forceThread = Thread(target=poll_force)
    #forceThread.start()
    while True:
        #ALL RED
        time.sleep(1)
